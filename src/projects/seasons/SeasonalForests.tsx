...
const mySketch = (p: p5) => {
  ...
  p.draw = () => {
    ...
    forest.leaves.forEach(row => row.forEach(l => {
      drawLeaf(l, 0.2, p.random(forest.fills))
    }));
  ...
  function drawLeaf({x, y, w, h, angle, start, stop}, probability, fill_c) {
  ...
  function drawToBuffer(circleBuffer, {x, y, r}, fills) {
    ...
    circleBuffer.fill(fills[p.random([3])]);
